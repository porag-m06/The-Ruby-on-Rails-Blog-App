...
RSpec.describe Post, type: :model do
  ...
  describe '#five_recent_comments' do
    ...
    let!(:older_comment) { Comment.create(user: user, post: post, text: 'Older Comment', created_at: 1.week.ago) }
    let!(:recent_comments) do
      5.times.map do
        Comment.create(user: user, post: post, text: 'Recent Comment', created_at: Time.zone.now)
      end
    end
